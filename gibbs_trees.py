...
class GibbsTrees(object):
    ...
    def hash_tree(self, tree, subtree=None):
        ...
        # convert the subtree to the tuple representation
        root = self.get_root(tree, subtree)
        res = self._mktree(tree, subtree, root)
...
class GibbsSampling(object):
    ...
    def __init__(self, gibbs_trees, tree_decoding, alfa, tree_depth_mean):
        ...
        # mean depth of tree
        self.tree_depth_mean = tree_depth_mean
    ...
    def p0tree(self, tree, subtree=None):
        ...
        res = 1.0  #poisson_probability(len(subtree), self.tree_len_mean)
        for node in subtree:
        ...
        print res, self.gibbs_trees.hash_tree(tree, subtree)
        return res
    ...
    def do_iteration(self):
        ...
        for tree_id in seq:
            ...
            while len(possible_picks) > 0:
                ...
                if is_not_boundary_node or is_not_roots_child:
                    break
            else:
                couldnt_break += 1
            ...
            if is_not_boundary_node:
                ...
                newp = self.ptree(curr_tree, curr_subtrees[old_group_id])
            ...
            # randomly pick a number and see whether it exceeds the boundary; if not, make the change
        ...
        ones_in_100 = 0
        for i in range(100):
            if len(self.tree_decoding.split_counts[i].values()) == 1:
                ones_in_100 += 1

        print 'ones:', ones_in_100
        print 'couldnt break', couldnt_break
...
class TreeDecoding(object):
    ...
    def get_partitioning(self, tree, groups):
        ...
        good_subtrees = {}
        node_marks = {}  # mark nodes whether they are present in a group or not

        # for each subtree, mark nodes whether they make up a linear segment or not
        for subtree in subtrees.values():
            min_node = min(subtree)
            max_node = max(subtree)

            if max_node - min_node == len(subtree) - 1:
                good_subtrees[min_node] = subtree
                node_mark = 1
            ...
            for node in subtree:
                node_marks[node] = node_mark

        # create the final partitioning of the phrase given by the tree
        # i.e. for each node, look whether it makes up a group; if not, just insert it
        # on its own, if yes, insert the whole group
        res = []
        tree_size = len(tree.parents.keys())
        node = 1
        while node < tree_size + 1:
            if node_marks[node] == 1:  # the node makes up a group
                subtree = good_subtrees[node]
                res += [tuple(sorted(subtree))]
                node = max(subtree) + 1
            else:  # the node is not in any group
                res += [(node,)]
                node += 1

        # return it as tuple so that it's hashable
        return tuple(res)
...
def main(fname):
    ...
    params = "e%d_n%s_alfa%.6f_tlm%d_i%d" % (exp_id, str(n_trees), alfa, tree_depth_mean, iters,)
    ...
    gs = GibbsSampling(gt, td, alfa=alfa, tree_depth_mean=tree_depth_mean)
    gs.do(iters, collect_start=0)
